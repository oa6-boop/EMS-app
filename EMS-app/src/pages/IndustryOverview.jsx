...
const toMAD = (v) => `${(Number(v || 0) * 10).toFixed(2)} MAD`;
...
function BarChart({ lines, metric, unit, color = "#4299e1" }) {
...
export default function IndustryOverview() {
  ...
  const [structure,    setStructure]    = useState({ lines:[], zones:[], plants:[] });
  ...
  const handleResolve = async (id) => {
    try { await resolveAlarm(id); loadData(); }
  ...
  const filteredAlarms = filterSev === "all" ? alarms
  ...
  return (
    <div className="overview-page">
      ...
      {/* Structure découverte automatiquement */}
      {(structure.plants.length > 0 || structure.zones.length > 0 || structure.lines.length > 0) && (
            ...
            {structure.plants.length > 0 && (
              <div className="carbon-card">
                ...
                {structure.plants.map(p => (
                  <span key={p} style={{ display:"block", fontSize:"0.8rem", color:"#2563eb" }}>{p}</span>
                ))}
              </div>
            )}
            {structure.zones.length > 0 && (
              <div className="carbon-card">
                ...
                {structure.zones.map(z => (
                  <span key={z} style={{ display:"block", fontSize:"0.8rem", color:"#7c3aed" }}>{z}</span>
                ))}
              </div>
            )}
            {structure.lines.length > 0 && (
              <div className="carbon-card">
                ...
                {structure.lines.map(l => (
                  <span key={l} style={{ display:"block", fontSize:"0.8rem", color:"#0891b2" }}>{l}</span>
                ))}
              </div>
            )}
            <div className="carbon-card">
              <h4>⚡ Active Meters</h4>
              <strong>{Object.keys(linesSummary).length}</strong>
            </div>
          </div>
        </section>
      )}
      ...
      <section className="section-block">
        ...
        <div className="kpi-grid">
          <div className="kpi-card">
            ...
            <span>Energy only · 1.40 MAD/kWh</span>
          </div>
          <div className="kpi-card">
          ...
          <>
                  ...
                  {Object.entries(linesSummary).map(([lineName, data]) => {
                    ...
                    return (
                      <tr key={lineName}>
                        ...
                        <td style={{ fontSize:"0.8rem", color:"#2563eb" }}>{plant}</td>
                        <td style={{ fontSize:"0.8rem", color:"#7c3aed" }}>{zone}</td>
            ...
            <div className="two-column-layout">
              <div className="panel-card">
                ...
                <BarChart lines={linesSummary} metric="avg_kw"     unit="kW"  color="#4299e1" />
              </div>
              <div className="panel-card">
                <div className="panel-head"><div><h2>CO₂ by Line</h2><p>kg CO₂ — {period}</p></div></div>
                <BarChart lines={linesSummary} metric="total_co2"  unit="kg"  color="#38a169" />
              </div>
              <div className="panel-card">
                <div className="panel-head"><div><h2>Cost by Line</h2><p>MAD — {period}</p></div></div>
                <BarChart lines={linesSummary} metric="total_cost" unit="MAD" color="#d69e2e" />
              </div>
              <div className="panel-card">
                <div className="panel-head"><div><h2>kWh by Line</h2><p>Cumulative — {period}</p></div></div>
                <BarChart lines={linesSummary} metric="latest_kwh" unit="kWh" color="#9f7aea" />
              </div>
            </div>
          </>
        ) : (
          <div style={{ textAlign:"center", padding:"2rem", background:"var(--bg-card)",
            borderRadius:"12px", border:"1px solid var(--border-color)", color:"var(--text-secondary)" }}>
            ⏳ Waiting for DataPlatform data...
          </div>
        )}
      </section>

      {/* Alarm Management — Flink only */}
      <section className="section-block">
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1rem" }}>
          <div className="section-title-wrap" style={{ margin:0 }}>
          ...
          <table>
            ...
            <tbody>
              {filteredAlarms.length > 0 ? (
                  ...
                  return (
                    <tr key={alarm.id}>
                      <td style={{ fontSize:"0.75rem", color:"var(--text-secondary)" }}>
                        {alarm.created_at ? new Date(alarm.created_at).toLocaleTimeString() : "—"}
                      </td>
                      <td style={{ fontSize:"0.8rem" }}>
                        <strong>{alarm.alarm_type}</strong>
                        <span style={{ marginLeft:"6px", background:"#eff6ff", color:"#2563eb",
                          borderRadius:"6px", padding:"1px 6px", fontSize:"0.7rem" }}>
                          ⚡ Flink
                        </span>
                      </td>
                      <td>
                        <span style={{ background:sc.bg, color:sc.text, border:`1px solid ${sc.border}`,
                          borderRadius:"8px", padding:"2px 8px", fontSize:"0.72rem", fontWeight:700, textTransform:"uppercase" }}>
                          {alarm.severity}
                        </span>
                      </td>
                      <td style={{ fontSize:"0.8rem", color:"#2563eb" }}>{alarm.plant || "Plant 1"}</td>
                      <td style={{ fontSize:"0.8rem", color:"#7c3aed" }}>{alarm.area  || "—"}</td>
                      ...
                      <td style={{ fontSize:"0.8rem", color:sc.text, fontWeight:700 }}>
                        {alarm.measured_value != null ? Number(alarm.measured_value).toFixed(2) : "—"}
                      </td>
                      <td>
                        <span style={{
                      ...
                      <td>
                        {alarm.status === "active" ? (
                          <button type="button" onClick={() => handleResolve(alarm.id)}
                            style={{ background:"#38a169", color:"#fff", border:"none",
                ...
                <tr>
                  <td colSpan="11" style={{ textAlign:"center", color:"var(--text-secondary)", padding:"2rem" }}>
                    {filterSev === "all"
                      ? "✅ No alarms from Flink. All parameters within range."
                      : `No ${filterSev} alarms.`}
                  </td>
                </tr>
              )}
