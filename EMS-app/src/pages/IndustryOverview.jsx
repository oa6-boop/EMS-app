...
const isCO2 = (name = "") => {
  ...
  return n.includes("co2") || n.includes("co₂") || n.includes("carbon") || n.includes("emission");
};

function MultiLineBarChart({ lines, metric, unit, color = "#4299e1" }) {
  const values = Object.entries(lines).map(([name, data]) => ({ name, value: parseFloat(data[metric] || 0) }));
      ...
      {values.map((item, i) => {
        ...
        return (
          <g key={item.name}>
            ...
            <text x={x+barW/2} y={y-4} textAnchor="middle" fontSize="9" fill="var(--text-main)" fontWeight="600">
              {item.value.toFixed(1)}
            </text>
            <text x={x+barW/2} y={H-8} textAnchor="middle" fontSize="9" fill="var(--text-secondary)">
...
export default function IndustryOverview() {
  ...
  const loadData = async () => {
    try {
      const [kpiResult, alarmResult] = await Promise.all([fetchIndustryKpis(), fetchIndustryAlarms()]);
      setKpis(kpiResult);
      setAlarms(alarmResult || []);
      setError("");
    } catch (err) { setError(err.message || "Failed to load industry data"); }
  };
  ...
  useEffect(() => { loadLinesSummary(); const iv = setInterval(loadLinesSummary, 15000); return () => clearInterval(iv); }, [period]);

  const handleResolve = async (alarmId) => {
    try { await resolveAlarm(alarmId); loadData(); }
    catch (err) { setError(err.message || "Failed to resolve alarm"); }
  };
  ...
  const filteredAlarms = filterSev === "all" ? alarms : alarms.filter(a => a.severity === filterSev || a.status === filterSev);
    ...
    <div className="overview-page">
      <div className="overview-header-row">
        <div>
          ...
          <p className="page-subtitle">Global KPIs — All production lines — Plant 1</p>
        </div>
      ...
      <section className="section-block">
        <div className="section-title-wrap"><h2>Plant-Level KPIs</h2><p>Aggregated from all production lines</p></div>
        <div className="kpi-grid">
          
          {/* Coût total — énergies seulement, CO₂ exclu */}
          <div className="kpi-card">
            <div className="kpi-icon gold">💰</div><div className="kpi-badge red">Live</div>
            <h3>{toMAD(kpis?.total_cost ?? totalCostAll)}</h3>
            <p>Total Cost (all lines)</p>
            <span>Energy only · CO₂ excluded · 1.40 MAD/kWh</span>
          </div>
          <div className="kpi-card">
            <div className="kpi-icon orange">〰</div><div className="kpi-badge amber">Live</div>
            <h3>{kpis?.peak_demand ?? 0} kW</h3>
            <p>Peak Demand</p><span>Highest recorded</span>
          </div>
          <div className="kpi-card">
            <div className="kpi-icon leaf">🌱</div><div className="kpi-badge green">Live</div>
            <h3>{totalCo2All.toFixed(3)} kg</h3>
            <p>Total CO₂ (all lines)</p><span>kWh × 0.718 ONEE</span>
          </div>
          <div className="kpi-card">
            ...
            <h3 style={{ color: highAlarms.length > 0 ? "#e53e3e" : "#38a169" }}>{highAlarms.length}</h3>
            <p>High Severity Alarms</p><span>Require immediate action</span>
          </div>
          <div className="kpi-card">
            ...
            <h3 style={{ color: mediumAlarms.length > 0 ? "#d69e2e" : "#38a169" }}>{mediumAlarms.length}</h3>
            <p>Medium Severity Alarms</p><span>Monitor closely</span>
          </div>
          <div className="kpi-card">
            <div className="kpi-icon emerald">⚡</div><div className="kpi-badge green">Live</div>
            <h3>{totalKwAll.toFixed(1)} kW</h3>
            <p>Total Active Power</p><span>All lines combined</span>
          </div>
          <div className="kpi-card">
            <div className="kpi-icon darkblue">✅</div>
            <div className="kpi-badge" style={{ background:"#38a169" }}>Live</div>
            <h3>{resolvedAlarms.length}</h3>
            <p>Resolved Alarms</p><span>Total resolved</span>
          </div>
        </div>
      </section>
          ...
          <>
            <div className="table-card" style={{ marginBottom:"1.25rem" }}>
              <table>
                <thead>
                  <tr>
                    <th>Production Line</th><th>Avg Power (kW)</th><th>Peak (kW)</th>
                    <th>Total kWh</th><th>Total Cost (MAD)</th><th>CO₂ (kg)</th>
                  ...
                  {Object.entries(linesSummary).map(([lineName, data]) => {
                    ...
                    return (
                      <tr key={lineName}>
                        ...
                        {/* Coût en MAD — CO₂ exclu */}
                        <td style={{ color:"#d69e2e", fontWeight:600 }}>{toMAD(data.total_cost || 0)}</td>
            ...
            <div className="two-column-layout">
              <div className="panel-card">
                <div className="panel-head"><div><h2>Power by Line</h2><p>Average kW — {period}</p></div></div>
                <MultiLineBarChart lines={linesSummary} metric="avg_kw"     unit="kW"  color="#4299e1" />
              </div>
              <div className="panel-card">
                <div className="panel-head"><div><h2>CO₂ by Line</h2><p>kg CO₂ — {period}</p></div></div>
                <MultiLineBarChart lines={linesSummary} metric="total_co2"  unit="kg"  color="#38a169" />
              </div>
              <div className="panel-card">
                <div className="panel-head"><div><h2>Cost by Line</h2><p>MAD — {period} — CO₂ excluded</p></div></div>
                <MultiLineBarChart lines={linesSummary} metric="total_cost" unit="MAD" color="#d69e2e" />
              </div>
              <div className="panel-card">
                <div className="panel-head"><div><h2>kWh by Line</h2><p>Cumulative — {period}</p></div></div>
                <MultiLineBarChart lines={linesSummary} metric="latest_kwh" unit="kWh" color="#9f7aea" />
              </div>
            </div>
          </>
        ) : (
          <div style={{ textAlign:"center", padding:"2rem", background:"var(--bg-card)", borderRadius:"12px", border:"1px solid var(--border-color)", color:"var(--text-secondary)" }}>
      ...
      <section className="section-block">
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1rem" }}>
          <div className="section-title-wrap" style={{ margin:0 }}>
            ...
            <p>
              ...
              {highAlarms.length > 0 && (
                <span style={{ color:"#e53e3e", fontWeight:700, marginLeft:"0.5rem" }}>⚠ {highAlarms.length} HIGH severity</span>
              )}
            </p>
          </div>
          <div style={{ display:"flex", gap:"0.4rem" }}>
            {["all","high","medium","active","resolved"].map(f => (
              <button key={f} type="button" onClick={() => setFilterSev(f)} style={{
                padding:"3px 10px", borderRadius:"20px",
                border:"1px solid var(--border-color)", cursor:"pointer",
                fontSize:"0.75rem", fontWeight: filterSev === f ? 700 : 400,
        ...
        <div className="table-card">
          <table>
            <thead>
              <tr><th>Time</th><th>Type</th><th>Severity</th><th>Line</th><th>Equipment</th><th>Message</th><th>Value</th><th>Limit</th><th>Status</th><th>Action</th></tr>
            </thead>
            <tbody>
              {filteredAlarms.length > 0 ? (
                filteredAlarms.slice(0, 50).map(alarm => {
                  ...
                  return (
                    <tr key={alarm.id}>
                      <td style={{ fontSize:"0.75rem", color:"var(--text-secondary)" }}>{alarm.created_at ? new Date(alarm.created_at).toLocaleTimeString() : "—"}</td>
                      <td style={{ fontSize:"0.8rem" }}><strong>{alarm.alarm_type}</strong></td>
                      <td>
                        <span style={{ background:sc.bg, color:sc.text, border:`1px solid ${sc.border}`, borderRadius:"8px", padding:"2px 8px", fontSize:"0.72rem", fontWeight:700, textTransform:"uppercase" }}>
                          {alarm.severity}
                        </span>
                      </td>
                      <td style={{ fontSize:"0.8rem" }}>{alarm.production_line}</td>
                      <td style={{ fontSize:"0.8rem" }}><strong>{alarm.equipment}</strong></td>
                      <td style={{ fontSize:"0.78rem", color:"var(--text-secondary)", maxWidth:"200px" }}>{alarm.message}</td>
                      <td style={{ fontSize:"0.8rem", color:sc.text, fontWeight:700 }}>{alarm.measured_value ?? "—"}</td>
                      <td style={{ fontSize:"0.8rem" }}>{alarm.limit_value ?? "—"}</td>
                      <td>
                        <span style={{ background: alarm.status === "active" ? "#fff5f5" : "#f0fff4", color: alarm.status === "active" ? "#e53e3e" : "#38a169", borderRadius:"8px", padding:"2px 8px", fontSize:"0.72rem", fontWeight:700 }}>
                          {alarm.status}
                        </span>
                      </td>
                      <td>
                        {alarm.status === "active" ? (
                          <button type="button" onClick={() => handleResolve(alarm.id)}
                            style={{ background:"#38a169", color:"#fff", border:"none", borderRadius:"6px", padding:"3px 10px", cursor:"pointer", fontSize:"0.75rem", fontWeight:600 }}>
                            ✓ Resolve
                          </button>
                        ) : (
                ...
                <tr><td colSpan="10" style={{ textAlign:"center", color:"var(--text-secondary)", padding:"2rem" }}>
