...
// CO₂ inclus dans le graphe — mais sans coût
function PrintableBarChart({ energies = [] }) {
  const filtered = energies; // ← CO₂ affiché dans le graphe
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H}
      ...
      {[0,1,2,3,4].map(i => (
        <line key={i} x1={20} y1={10+i*(chartH/4)} x2={W-10} y2={10+i*(chartH/4)}
        ...
        return (
          <g key={energy.id || i}>
            <rect x={x} y={y} width={barW} height={barH}
              fill={BAR_COLORS[i % BAR_COLORS.length]} rx="3" />
            <text x={x+barW/2} y={y-4} textAnchor="middle" fontSize="9" fill="#4a5568">
              {energy.value.toFixed(1)}
            </text>
            <text x={x+barW/2} y={H-5} textAnchor="middle" fontSize="8" fill="#718096">
...
export default function ReportsAnalytics({
  ...
  selectedEnergyNames = [], totalCost = 0, peakKw = 0, totalCo2 = 0,
}) {
  ...
  const handleExportCSV = () => {
    ...
    const metaRows = [
      ...
      [`Generated: ${now}`],
      [`Total Cost: ${toMAD(totalCost)}`, `Peak Demand: ${Number(peakKw).toFixed(1)} kW`, `Total CO2: ${Number(totalCo2).toFixed(3)} kg`],
      [],
    ];
    const headers = ["Equipment","Area","Energy Type","Value","Unit","Voltage (V)","Power Factor","Cost (MAD)","CO2 (kg)","Timestamp"];
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      await shareReportToConversation(
        { conversation_id: Number(selectedConversationId), file_name:`Report-${selectedLineLabel}.pdf`, file_url:"#generated-report" },
        token
      );
      setShowShareModal(false); setSelectedConversationId(""); setShareError("");
      setShareSuccess("Report shared successfully.");
      setTimeout(() => setShareSuccess(""), 3000);
    } catch (e) { setShareError(e.message || "Failed to share"); }
  };

  const totalKw = energies.filter(e => e.unit === "kW").reduce((s, e) => s + e.value, 0);
  const realTotalCost = totalCost > 0
    ? totalCost
    : energies.filter(e => !isCO2(e.name)).reduce((s, e) => s + (e.cost || 0), 0);
  ...
  return (
    <div className="overview-page reports-export-root" id="reports-export-root">
      ...
      <div className="section-title-wrap reports-header-row">
        ...
        <div className="reports-actions-row no-print"
      ...
      <section className="section-block">
        ...
        <div className="carbon-kpis">
          <div className="carbon-card">
            ...
            <span>Energy only — CO₂ not billed</span>
          </div>
          <div className="carbon-card">
          ...
          <div className="carbon-card">
            ...
            <span>Average · Nominal 415V</span>
          </div>
          <div className="carbon-card">
      ...
      <section className="section-block">
        ...
        <div className="table-card">
          <table>
            <thead>
              <tr>
                <th>Equipment</th><th>Area</th><th>Energy</th>
                <th>Value</th><th>Forecast +1h</th><th>Unit</th>
                <th>Voltage</th><th>Power Factor</th>
                <th>Cost (MAD)</th><th>CO₂ (kg)</th>
      ...
      {/* Graphe Distribution + Analytics */}
      <div className="two-column-layout">
        <section className="panel-card">
          <div className="panel-head">
            <div>
              <h2>Consumption Distribution</h2>
              {/* CO₂ affiché dans le graphe — sans coût */}
              <p>Energy portfolio — CO₂ shown in kg</p>
            </div>
          </div>

          {/* Graphe avec CO₂ inclus */}
          <PrintableBarChart energies={energies.slice(0, 5)} />

          {/* Légende avec indication kg pour CO₂ */}
          <div style={{ display:"flex", gap:"0.5rem", flexWrap:"wrap", marginTop:"0.5rem" }}>
            {energies.slice(0, 5).map((e, i) => (
              <span key={e.id || i}
                ...
                {isCO2(e.name) && (
                  <span style={{
                    fontSize:"0.65rem", color:"#38a169",
                    background:"#f0fff4", border:"1px solid #c6f6d5",
                    borderRadius:"4px", padding:"0px 4px",
                  }}>
                    kg 
                  </span>
                )}
      ...
      {/* Cost Analysis — CO₂ exclu des coûts */}
      <section className="section-block">
        <div className="section-title-wrap">
        ...
        <div className="carbon-kpis">
          {energies.filter(e => !isCO2(e.name)).length > 0 ? (
            ...
            <div className="carbon-card"><h4>No data</h4><strong>—</strong><span>Waiting...</span></div>
          )}
        </div>
      ...
      <div className="print-footer"
        style={{ borderTop:"1px solid #e2e8f0", paddingTop:"0.75rem", marginTop:"1.5rem" }}>
        <p style={{ color:"#718096", fontSize:"0.78rem", textAlign:"center" }}>
          EMS Report — {selectedLineLabel} — {new Date().toLocaleString()} —
          CO₂: 0.718 kgCO₂/kWh (ONEE Morocco) — Costs in MAD — CO₂ shown in kg, not billed
        </p>
      </div>
        ...
        <div className="forgot-modal-overlay">
          <div className="forgot-modal">
            ...
            <div className="forgot-modal-actions">
              ...
              <button type="button" className="cancel-forgot-btn"
