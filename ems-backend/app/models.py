...
# ─── Tarifs énergies — gérés par l'admin ─────────────────────────────────────
class EnergyRate(Base):
    __tablename__ = "energy_rates"

    id          = Column(Integer,  primary_key=True, index=True)
    energy_name = Column(String,   unique=True, nullable=False, index=True)
    rate_mad    = Column(Float,    nullable=False)
    unit        = Column(String,   default="kWh", nullable=False)
...
class User(Base):
    ...
    role            = Column(String,   default="management", nullable=False)
